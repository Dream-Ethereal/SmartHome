...
void *check(void *arg)
{
	while (1)
	{
		...
		if(threshold_value[0] < cur_data[0] && mode == OFF)
			led_control(led1_fd_tg, led1_fd_bn, ON);
		else if(threshold_value[0] >= cur_data[0] && mode == OFF)
			led_control(led1_fd_tg, led1_fd_bn, OFF);

		//当当前温度超过阈值的时候，并且模式为自动模式，进行LED2亮
		if(threshold_value[1] < cur_data[3] && mode == OFF)
			led_control(led2_fd_tg, led2_fd_bn, ON);
		else if(threshold_value[1] >= cur_data[3] && mode == OFF)
